...
Default: one day before the newest household we already hold, so a nightly run
re-reads a little overlap instead of risking a gap. Any job can pass an explicit
from_date instead; EPOCH means "everything".
"""
...
def window_start(subject_type=None, from_date=None):
    ...
    if from_date:
        ...
        start = day_before(latest_submission(subject_type) or timezone.now())
