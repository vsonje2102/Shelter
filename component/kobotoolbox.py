...
def get_household_analysis_data(city, slum_code, question_fields, kobo_survey=''):
    ...
    '''
    ALWAYS_CALCULATE_FIELDS = ['Functioning of the structure']
    ...
    for household, list_record in grouped_records:
        ...
        for field in question_fields:
            # print("Processing Field:", field, " for Household No:", household_no)
            ...
            if field != "" and field in record and record[field] == record[field]:
                ...
                if 'Type_of_structure_occupancy' in record and record['Type_of_structure_occupancy'] != 'Occupied house':
                    if field == 'Type_of_structure_occupancy' or field in ALWAYS_CALCULATE_FIELDS: # here we will check only for occupancy field.
                        data = record[field]
...
def get_kobo_RIM_report_detail(city, slum_code, kobo_survey=''):
    ...
    slum_qs = Slum.objects.filter(shelter_slum_code = slum_code)
    ...
    if 'Toilet' in data:
        ...
        for i in toilet_info:
            ...
            if "toilet_comment" in i and i["toilet_comment"]:
                output["toilet_comment"] = (output.get("toilet_comment","") + " ; " + i["toilet_comment"]).strip(" ;")
